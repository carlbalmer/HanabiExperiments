...
class MultiAgentHanabiEnv(MultiAgentEnv):
    ...
    def __init__(self, env_config):
        ...
        n_actions = 2 * env_config["hand_size"] + (env_config["colors"] + env_config["ranks"]) * (env_config["players"] - 1)
        self.action_space = LegalActionDiscrete(n_actions, self)

        sample_obs = numpy.array(self.state["player_observations"][0]["vectorized"])
        self.observation_space = Box(low=sample_obs.min(), high=sample_obs.max(), shape=sample_obs.shape, dtype=sample_obs.dtype)
    ...
    def step(self, action_dict):
        ...
        reward_dict = {next_player: reward}
        done_dict = {"__all__": done}
        if done:
            obs_dict = {0: next_player_obs,
                        1: next_player_obs,
                        2: next_player_obs,
                        3: next_player_obs}
...
class LegalActionDiscrete(Discrete):
    ...
    def contains(self, x):
        if isinstance(x, int):
            as_int = x
        elif isinstance(x, (numpy.generic, numpy.ndarray)) and (x.dtype.kind in numpy.typecodes['AllInteger'] and x.shape == ()):
            as_int = int(x)
        else:
            return False

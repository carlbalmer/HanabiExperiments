...
def rollout(agent,
    ...
    while keep_going(steps, num_steps, episodes, num_episodes):
        ...
        while not done and keep_going(steps, num_steps, episodes,
            ...
            actions.append(str(a_action))
            steps += 1
            obs = next_obs
        saver.end_rollout()
        print("Episode #{}: reward: {}".format(episodes, reward_total))
        episode_rewards.append(str(reward_total))
        episode_actions.append(actions)
        if done:
            episodes += 1
    with open(os.path.join(Path(saver.outfile).parent, "rollout_episode_rewards.txt"), 'w') as f:
        f.write('\n'.join(episode_rewards))
    with open(os.path.join(Path(saver.outfile).parent, "rollout_episode_actions.txt"), 'w') as f:
        f.write('\n'.join([','.join(actions) for actions in episode_actions]))
...
if __name__ == "__main__":
    ...
    keep_pkl = False
    default_args = {"episodes": 1000, "no_render": True, "config": {}, "env": False, "steps": 0, "use_shelve": False, "track_progress": False, "save_info": True, "monitor": False}

    checkpoints = Path('').rglob('checkpoint-*[0-9]')
    ...
    for checkpoint in checkpoints:
        ...
        experiment_states = list(checkpoint.parents[2].glob('experiment_state*.json'))
        if len(experiment_states) == 0:
            print("no experiment_state" + str(checkpoint))
        else:
            with open(experiment_states[0]) as f:
                trainable_name = json.load(f)["checkpoints"][0]["trainable_name"]
